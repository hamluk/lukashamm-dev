...
function SegmentedControl({ activeView, onUpdate }: Props) {
  ...
  return (
    <div className="flex flex-row  items-center text-sm lg:text-base font-semibold inset-shadow-sm rounded-4xl bg-gray-100 gap-2 p-2">
      <button
        onClick={() => onUpdate("form")}
        className={classNames(
          "rounded-4xl transition-all min-w-35 duration-300 ease-in-out px-3 py-3 hover:cursor-pointer",
          {
      ...
      <button
        ...
        className={classNames(
          "rounded-3xl transition-all min-w-35 duration-300 ease-in-out px-3 py-3 hover:cursor-pointer",
          {
            "bg-gray-100 text-dark-blue-2 hover:bg-dark-blue-2/50 hover:text-white":
              activeView === "form",
